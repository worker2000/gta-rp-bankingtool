...
ob_start();
/**
 * PSB Kreditverwaltung - Kreditnehmer bearbeiten
 */
require_once __DIR__ . '/../../includes/header.php';
Auth::requirePermission('borrowers', 'edit');
...
$borrower = Database::fetchOne("SELECT * FROM borrowers WHERE id = ?", [$id]);
if (!$borrower) {
...
$pageTitle = 'Bearbeiten: ' . $borrower['last_name'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf()) {
        ...
        // Daten sammeln
        $fields = ['salutation', 'first_name', 'last_name', 'date_of_birth', 'phone', 'email',
                   'employer', 'company', 'weekly_income', 'total_assets', 'bank_account_iban', 'bank_account_holder', 'notes'];
        ...
        if (empty($errors)) {
            ...
            Database::update('borrowers', $updateData, 'id = ?', [$id]);
            AuditLog::log('UPDATE', 'borrower', $id, $oldData, $updateData);

            setFlash('success', 'Kreditnehmer erfolgreich aktualisiert.');
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <a href="<?= APP_URL ?>/pages/borrowers/view.php?id=<?= $id ?>" class="text-muted text-decoration-none">
            <i class="bi bi-arrow-left me-2"></i>Zurück
        </a>
        <h4 class="mt-2 mb-0">
            <i class="bi bi-pencil me-2"></i>Kreditnehmer bearbeiten
        </h4>
        ...
        <form method="POST" action="">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 mt-4">
                    ...
                    <a href="<?= APP_URL ?>/pages/borrowers/view.php?id=<?= $id ?>" class="btn btn-outline-secondary">Abbrechen</a>
                </div>
            </div>
        </form>
    </div>
